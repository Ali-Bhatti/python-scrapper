...
Tests basic functionality with a sample ASIN and URL
"""
...
def test_scraper():
    ...
    try:
        ...
        # Ensure CLI accepts a URL as the first argument
        print("\nTesting CLI with URL as positional argument...")
        result = subprocess.run(
            [sys.executable, 'amazon_review_scraper.py', test_url, '--max-pages', '1', '--format', 'json'],
            capture_output=True, text=True
        )
        if result.returncode in (0, 1):
            print("✅ CLI accepted URL positional argument")
        else:
            print("❌ CLI failed to accept URL positional argument")
